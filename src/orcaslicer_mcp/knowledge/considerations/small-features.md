...
- IF the user's situation says the model has thin fins, blades, or single-bead-width walls THEN weigh enabling `detect_thin_wall` and a narrower line width for those features specifically, rather than lowering line width globally, because a global change would also unnecessarily thin normal-thickness walls.
